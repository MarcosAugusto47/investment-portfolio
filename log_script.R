...
df = read_xls("data/bronze/InfoCEI-01_04_2021.xls")
df = formatting(df, columns, numeric_columns)
df2 = read_xls("data/bronze/InfoCEI-01_04_2021-14_04_2022.xls")
df2 = formatting(df2, columns, numeric_columns)
df = rbind(df, df2)
df_events = read_excel("data/bronze/stock_events.xlsx")
df_events = stock_events_treatment(df_events)
df_log = create_log(df, df_events)
write.csv(df_log, "data/silver/log.csv", row.names = FALSE)
